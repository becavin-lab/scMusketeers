...
do
    ...
    #sbatch ${gpu_sbatch} --output ${log_out} --job-name t1_allmod_$dataset ${sh_file} $dataset $class_key $batch_key True
    sbatch ${cpu_sbatch} --output ${log_out} --job-name t1_allmod_$dataset ${sh_file} $dataset $class_key $batch_key False
done
